...
import weixinInfoApi from '@/api/weixin'
export default {
  data() {
    return {
      orderId: null,
  ...
  methods: {
    ...
    cancelOrder() {
        this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
          ...
          orderInfoApi.cannerOrder(this.orderId).then(resp =>{
    ...
    queryPayStatus(orderId) {
      weixinInfoApi.queryPayStatus(orderId).then(response => {
    ...
    pay(){
      ...
      //请求后端
      weixinInfoApi.createNative(this.orderId).then(resp=>{
    ...
    init() {
      orderInfoApi.detail(this.orderId).then(response => {
